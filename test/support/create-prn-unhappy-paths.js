 ...
 * @param {string} config.summaryLogFilePath - summary-log fixture uploaded to seed a waste balance, so a within-balance draft passes the create-form pre-check
...
export async function runCreatePrnUnhappyPaths(
  ...
  // Seed a waste balance so within-balance drafts can be created (the discard
  // and field-validation flows below depend on reaching the check page); the
  // over-balance case is asserted at the create step further down.
  ...
  // Discard this draft to return to the Create page for the validation checks.
  // Previously a confirm here bounced back to Create off a zero balance; with a
  // seeded balance the confirm would succeed, so we discard to get there.
  await checkBeforeCreatingPrnPage.discardAndStartAgainLink().click()
  ...
  // Insufficient balance is caught at the create step: entering a tonnage above
  // the available balance re-renders the Create page with the error, instead of
  // proceeding to the check page.
