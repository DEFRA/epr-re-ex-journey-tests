...
const AUTH_PASSWORD = 'testpassword'

const ORGANISATIONS_BODY = JSON.stringify({
  organisations: [
    {
...
// No Express-specific behaviour was actually in use (one GET route, no body
// parsing) - rewritten on the built-in http module so the mock's Docker image
// needs no npm install at build time, just a COPY of this file.
...
server.listen(3020, '0.0.0.0', () => {
  ...
  console.log(
    'Test with: curl -u testuser:testpassword http://localhost:3020/waste-organisations/organisations'
  )
})
